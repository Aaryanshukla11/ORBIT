...
class CurrentStateObserver:
    ...
    async def observe(
        ...
        if not target_app_name and objective and objective.target_entities:
            target_app_name = objective.target_entities[0].lower()
        ...
        if target_app_name:
            active_proc = desktop_obs.foreground_window.process_name if desktop_obs.foreground_window else None
            if self._matches_app(active_title, active_class, target_app_name, active_proc):
                ...
                for win in visible_windows:
                    if self._matches_app(win.get("title", ""), win.get("class_name", ""), target_app_name, win.get("process_name")):
                        target_app_exists = True
        ...
        # Build and attach 4-Tier UnifiedWorldState (Phase 2D)
        from orbit.runtime.cognitive.world_state import WorldStateBuilder
        obs.world_state = WorldStateBuilder.build_world_state(obs)
        return obs

    def _matches_app(self, title: Optional[str], class_name: Optional[str], target: str, process_name: Optional[str] = None) -> bool:
        """Helper to match window title/class/process against target app name."""
        ...
        if p_low in ("msedge.exe", "chrome.exe", "brave.exe", "firefox.exe", "mspaint.exe", "calc.exe", "notepad.exe"):
            pass
        elif any(ed in t_low for ed in ("antigravity ide", "visual studio code", "vscode", "sublime text", "pycharm")) or p_low in ("antigravity ide.exe", "code.exe", "devenv.exe"):
            return False
        ...
        if tgt_low in ("paint", "mspaint"):
            return (("paint" in t_low and not t_low.endswith(".py") and not t_low.endswith(".ts") and not t_low.endswith(".js")) 
                    or "mspaintapp" in c_low or "msppaint" in c_low or "mspaint" in p_low)
        if tgt_low in ("edge", "msedge", "microsoft edge"):
            return "edge" in t_low or "msedge" in t_low or "edge" in c_low or "msedge" in p_low
        if tgt_low in ("notepad", "notepad.exe"):
            return "notepad" in t_low or "notepad" in c_low or "notepad" in p_low
        if tgt_low in ("calculator", "calc"):
            return "calculator" in t_low or "calc" in t_low or "calculator" in p_low
        if tgt_low in ("chrome", "google chrome"):
            return "chrome" in t_low or "chrome" in c_low or "chrome" in p_low

        return tgt_low in t_low or tgt_low in c_low or tgt_low in p_low
