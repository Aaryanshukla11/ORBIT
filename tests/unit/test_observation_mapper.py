"""Unit tests for observation model mapping functions."""
...
class MockConfidenceLevel(str, Enum):
    CONFIRMED = "CONFIRMED"
...
@dataclass
class MockWindowObservation:
    hwnd: int
    process_id: int
    process_name: str
    window_title: str
    extended_bounds: MockRect
    client_bounds: MockRect
    is_visible: bool
    is_minimized: bool
    is_maximized: bool
    is_foreground: bool
...
@dataclass
class MockUIElementObservation:
    element_id: str
    evidence_source: str
    name: Optional[str]
    role: str
    ...
    timestamp_ns: int
    generation_id: int
    confidence: MockConfidenceLevel
    class_name: Optional[str] = None


@dataclass
class MockDetectedTarget:
    target_id: str
    name: Optional[str]
    role: str
    physical_bounds: MockRect
    window_relative_bounds: Optional[MockRect]
...
class MockSnapshot:
    ...
    desktop_geometry: MockRect
    foreground_window: Optional[MockWindowObservation]
    windows: Tuple[MockWindowObservation, ...]
    visual_evidence: Tuple[Any, ...]
    accessibility_evidence: Tuple[Any, ...]
    detected_targets: Tuple[Any, ...]
    confidence: MockConfidenceLevel
    conflicts: Tuple[Any, ...]
    invalidation_state: str = "NONE"
    is_stale: bool = False
    telemetry: dict = field(default_factory=dict)


def test_map_rect_to_bounding_box():
    rect = MockRect(left=100, top=200, right=500, bottom=600)
...
def test_map_confidence_level():
    assert map_confidence_level(MockConfidenceLevel.CONFIRMED) == ObservationConfidence.CONFIRMED
    assert map_confidence_level(MockConfidenceLevel.PARTIALLY_CONFIRMED) == ObservationConfidence.PARTIALLY_CONFIRMED
    assert map_confidence_level(MockConfidenceLevel.VISUAL_FALLBACK) == ObservationConfidence.VISUAL_FALLBACK
    assert map_confidence_level(MockConfidenceLevel.CONFLICTING) == ObservationConfidence.CONFLICTING
    assert map_confidence_level(MockConfidenceLevel.UNAVAILABLE) == ObservationConfidence.UNAVAILABLE
    assert map_confidence_level(None) == ObservationConfidence.UNAVAILABLE


def test_map_window_observation():
    proto_win = MockWindowObservation(
        hwnd=0x1234,
        process_id=5678,
        process_name="notepad.exe",
        window_title="Untitled - Notepad",
        extended_bounds=MockRect(100, 100, 900, 700),
        client_bounds=MockRect(108, 130, 892, 692),
        is_visible=True,
        is_minimized=False,
        is_maximized=False,
        is_foreground=True,
...
def test_map_ui_element_missing_fields_preserved():
    proto_elem = MockUIElementObservation(
        element_id="elem_001",
        ...
        control_type="Button",
        automation_id=None,
        bounds=MockRect(200, 300, 300, 340),
        is_enabled=True,
        is_focused=False,
        is_offscreen=False,
        timestamp_ns=time.perf_counter_ns(),
        generation_id=1,
        confidence=MockConfidenceLevel.CONFIRMED,
        class_name=None,
    )
...
def test_map_detected_target():
    proto_target = MockDetectedTarget(
        target_id="target_btn",
        name="Submit",
        role="Button",
        physical_bounds=MockRect(400, 500, 520, 540),
        window_relative_bounds=MockRect(50, 60, 170, 100),
...
def test_map_full_prototype_snapshot():
    proto_snap = MockSnapshot(
        generation_id=42,
        timestamp_ns=time.perf_counter_ns(),
        capture_duration_ms=18.5,
        desktop_geometry=MockRect(0, 0, 1920, 1080),
        foreground_window=MockWindowObservation(
            hwnd=0x999,
            process_id=111,
            process_name="code.exe",
            window_title="ORBIT",
            extended_bounds=MockRect(0, 0, 1920, 1080),
            client_bounds=MockRect(0, 0, 1920, 1080),
            is_visible=True,
            is_minimized=False,
            is_maximized=True,
            is_foreground=True,
        ...
        is_stale=False,
        telemetry={"provider": "NativeAdapter"},
    )
    ...
    assert mapped.telemetry["provider"] == "NativeAdapter"
