...
async def main():
    ...
    if paint_hwnd:
        # Get window rect and click title bar
        rect = ctypes.wintypes.RECT()
        ctypes.windll.user32.GetWindowRect(paint_hwnd, ctypes.byref(rect))
        click_x = rect.left + 150
        click_y = rect.top + 15
        print(f"Clicking Paint title bar at ({click_x}, {click_y})...")
        pyautogui.click(click_x, click_y)
        await asyncio.sleep(0.5)

    # Click canvas and draw circle
    pyautogui.moveTo(500, 500)
    pyautogui.dragTo(650, 650, duration=0.4, button='left')
    await asyncio.sleep(0.5)
    ...
    fg_save, visible_save = observer.observe_windows()
    print("FG after Ctrl+S:", fg_save.title if fg_save else None, "Class:", fg_save.window_class if fg_save else None)
    for w in visible_save:
    ...
    # In case overwrite confirmation prompt appears
